...
import HybridStageRoot from "./HybridStageRoot";
...
export default class HybridBackend extends RenderBackend {
	constructor(stage) {
		super();
	...
	renderRoot(force = false) {
		...
		this.root.render(
			React.createElement(HybridStageRoot, {
				width,
				height,
		...
		if (!this.mountPromise) {
			this.mountPromise = loadFiberModule()
				...
				.catch((error) => {
					...
					console.error("[render] Failed to mount hybrid root:", error);
				})
				.finally(() => {
					if (!this.root) {
						this.mountPromise = null;
	...
	render(frameData) {
		...
		if (!this.r3fAvailable) {
			this.setRenderMode("hybrid-error");
			return;
		}
		...
		if (!this.root) {
			this.mountRoot();
			this.setRenderMode("hybrid-initializing");
			return;
		}

		this.setRenderMode("hybrid");
		this.renderRoot();
	}
